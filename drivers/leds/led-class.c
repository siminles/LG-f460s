...
#ifdef CONFIG_LEDS_PMA8084_EMOTIONAL
static ssize_t get_pattern(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int count = 0;
...
static ssize_t set_pattern(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = -EINVAL;
	int pattern_num = 0;
	...
	if (chg_psy) {
		chg_psy->get_property(chg_psy,
				POWER_SUPPLY_PROP_PRESENT, &chg_online);
	} else {
...
#if defined(CONFIG_LEDS_WINDOW_COLOR)
static ssize_t get_window_color(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, " - Window Color is '%s' \n", win_color);
}

static ssize_t set_window_color(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = -EINVAL;
	...
	if (strstr(color, "white") != NULL) {
		...
		memcpy(win_color, "white", sizeof("white"));
		window_color = WINDOW_COLOR_WH;
						...
						const char *buf, size_t size)
{
	...
	printk("\n");
	printk(KERN_INFO "[RGB LED] [RED] duty_ms:%d, pause_lo:%d, "
			"start:%d, length:%d \n",
		   red_duty, red_pause_lo, red_start_idx, red_length);
	printk(KERN_INFO "[RGB LED] [GREEN] duty_ms:%d, pause_lo:%d, "
			"start:%d, length:%d \n",
		   green_duty, green_pause_lo, green_start_idx, green_length);
	printk(KERN_INFO "[RGB LED] [BLUE] duty_ms:%d, pause_lo:%d, "
			"start:%d, length:%d \n",
...
static ssize_t confirm_blink_pattern(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int count = 0;
...
static ssize_t make_blink_pattern(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
...
static DEVICE_ATTR(blink_patterns, 0644,
		confirm_blink_pattern, make_blink_pattern);

static ssize_t confirm_onoff_pattern(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, LED_BUFF_SIZE, "0x%06u\n", onoff_rgb);
}

static ssize_t make_onoff_pattern(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "0x%06x", &onoff_rgb) != 1)
		printk(KERN_INFO "[RGB LED] make_onoff_pattern() bad arguments ");
...
static DEVICE_ATTR(onoff_patterns, 0644,
		confirm_onoff_pattern, make_onoff_pattern);
...
int led_pattern_sysfs_register(void)
{
	...
	pattern_sysfs_dev = device_create(lg_rgb,
			NULL, 0, NULL, "use_patterns");
