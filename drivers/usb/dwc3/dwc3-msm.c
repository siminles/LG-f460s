...
#define ROUND_USBIN_DWC(x) (x/100)
static int
get_prop_usbin_voltage_now(struct dwc3_msm *mdwc)
{
	...
	if (IS_ERR_OR_NULL(mdwc->vadc_dev)) {
		mdwc->vadc_dev = qpnp_get_vadc(mdwc->dev, "mpp5_usbin");
		if (IS_ERR(mdwc->vadc_dev))
			return PTR_ERR(mdwc->vadc_dev);
	}

	rc = qpnp_vadc_read(mdwc->vadc_dev, P_MUX5_1_3, &results);
	if (rc) {
		pr_err("Unable to read usbin adc rc=%d\n", rc);
		return 0;
	} else {
		usb_vol = results.physical * 147; /* 1.47 = (R1+R2)/(R1)*100 */
		usb_vol = ROUND_USBIN_DWC(usb_vol);
		pr_info("usb_in voltage: %d\n", usb_vol);
		return usb_vol;
	}
}
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		if (mdwc->otg_xceiv && !mdwc->ext_inuse &&
			...
			dev_info(mdwc->dev, "%s : set bsv (%d)",__func__, val->intval);
			/*
...
static void dwc3_ext_notify_online(void *ctx, int on)
{
	...
	if (on) {
		...
		dev_info(mdwc->dev, "%s:set bsv (%d)", __func__, mdwc->vbus_active);
...
#ifdef CONFIG_LGE_PM
		queue_delayed_work(system_nrt_wq,
			&mdwc->id_work, msecs_to_jiffies(100));
#else
...
#ifdef CONFIG_LGE_PM
		mdwc->usb_psy.set_event_property =
				dwc3_msm_power_set_event_property_usb;
#endif
