...
static struct touch_platform_data *get_dts_data(struct device *dev)
{
	...
	of_property_read_string(np, "fw_image_s3528_a0",
...
static int touch_resume(struct device *dev)
{
	...
	if (atomic_read(&ts->state.power_state) == POWER_WAKE) {
		TOUCH_INFO_MSG("%s, %d : Power is already ON\n",
				__func__, __LINE__);
		return 0;
	}
		...
		const struct i2c_device_id *id)
{
	...
	cc = strlen(ts->pdata->inbuilt_fw_name_s3528_a1);
	len_a1_suntel = strlen(ts->pdata->inbuilt_fw_name_s3528_a1_suntel);

	strncpy(ts->fw_info.fw_path, ts->pdata->inbuilt_fw_name, len);
	strncpy(ts->fw_info.fw_path_s3621,
			ts->pdata->inbuilt_fw_name_s3621, rc);
	strncpy(ts->fw_info.fw_path_s3528_a1,
			ts->pdata->inbuilt_fw_name_s3528_a1, cc);
	strncpy(ts->fw_info.fw_path_s3528_a1_suntel,
