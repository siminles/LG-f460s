...
void synaptics_ts_new_function(struct synaptics_ts_proximity_fn *prox_fn,
		bool insert)
{
	...
	if (power_state == POWER_ON || power_state == POWER_WAKE) {
		...
		if (!(strncmp(ts->fw_info.fw_product_id, "PLG391", 6))) {
			lower_img = get_limit(TxChannelCount,
					...
					*ts->client,
					ts->pdata,
					"LowerImageLimitSuntel",
					LowerImage);
			upper_img = get_limit(TxChannelCount,
					RxChannelCount,
					*ts->client,
					ts->pdata,
					"UpperImageLimitSuntel",
					UpperImage);
		} else {
			lower_img = get_limit(TxChannelCount,
					RxChannelCount,
...
static ssize_t show_tci(struct i2c_client *client, char *buf)
{
	...
	synaptics_ts_page_data_read(client, LPWG_PAGE, LPWG_TAPCOUNT_REG, 5,
			buffer);
...
static int read_page_description_table(struct i2c_client *client)
{
	...
	DO_SAFE(synaptics_ts_page_data_read(client, 0x02, EXIST_OFFSET, 1,
				&interrupt_cust), error);
	/* means fw version before v1.12 */
	if (interrupt_cust == F1A_EXIST)
		int_mask_cust = 0x40;
	else
		int_mask_cust = 0x20;
...
static int get_ic_info(struct synaptics_ts_data *ts)
{
	...
	if (!(strncmp(ts->fw_info.fw_product_id, "PLG313", 6)) ||
		...
		if (ts->fw_info.family && ts->fw_info.fw_revision) {
			if (!strncmp(ts->fw_info.fw_product_id, "PLG391", 6)) {
				rc = request_firmware(&fw_entry,
						ts->pdata->inbuilt_fw_name_s3528_a1_suntel,
						&ts->client->dev);
				ts->fw_flag = S3528_A1_SUN;
			} else {
				rc = request_firmware(&fw_entry,
						ts->pdata->inbuilt_fw_name_s3528_a1,
						&ts->client->dev);
				ts->fw_flag = S3528_A1;
			}
		} else {
		...
		rc = request_firmware(&fw_entry,
				ts->pdata->inbuilt_fw_name_s3621,
				&ts->client->dev);
		if (rc != 0) {
			TOUCH_ERR_MSG("request_firmware() failed %d\n", rc);
			goto error;
		...
		const struct touch_data *prev_data)
{
	...
	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x04, 1,
					&buf1) < 0), error);
	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x05, 1
					, &buf2) < 0), error);
	im = (buf2 << 8) | buf1;
	im_sum += im;

	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x08, 1,
					&cns) < 0), error);
	cns_sum += cns;

	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x09, 2,
					buffer) < 0), error);
	cid_im = (buffer[1]<<8)|buffer[0];
	cid_im_sum += cid_im;

	DO_SAFE((synaptics_ts_page_data_read(client,
					ANALOG_PAGE, 0x0A, 2, buffer) < 0),
			error);
...
int compare_fw_version(struct i2c_client *client,
	...
	if (ts->fw_info.fw_version[0] > 0x40) {
		...
		if (!(ts->fw_info.fw_version[3] & 0x80)) {
			/* temporary code to support HW team
			 if FW in IC is Test Ver., no update FW.*/
			if ((ts->fw_info.fw_version[3] & 0x7F) > 7) {
				TOUCH_DEBUG(DEBUG_BASE_INFO,
						"[Touch] %s : FW version is Test Version.\n",
						__func__);
				goto matched;
			} else { /*Possible to upgrade FW below v0.07*/
				TOUCH_DEBUG(DEBUG_BASE_INFO,
						"[Touch] %s : FW version is Test Version which is below v0.07\n",
						__func__);
				return 1;
			}
		}
...
enum error_type synaptics_ts_suspend(struct i2c_client *client)
{
	...
	u8 finger_amplitude_threshold[2] = { 0x29, 0x29 };
	u8 thermal_update_interval[2] = { 0x00, 0x10 };
	...
	DO_SAFE(touch_i2c_write(client, f12_info.ctrl_reg_addr[15], 2,
				&finger_amplitude_threshold[0]), error);
	DO_SAFE(synaptics_ts_page_data_write(client, ANALOG_PAGE,
				THERMAL_UPDATE_INTERVAL_REG, 2,
				thermal_update_interval), error);
...
static int set_doze_param(struct synaptics_ts_data *ts, int value)
{
	...
	buf_array[4] = 0x05;  /* Timer 1 */
	buf_array[5] = 0x05;  /* Max Active Duration Timeout */

	if (max_active_duration != buf_array[3])
