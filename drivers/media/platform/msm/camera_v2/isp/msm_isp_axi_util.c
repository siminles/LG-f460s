	...
	if (buf && ts) {
		...
		if (stream_info->buf_divert && rc == 0 &&
			...
			if (rc == 0) {
				...
				printk("%s: ping pong mismatch, recover after stop and restart\n", __func__);
...
/*                                                                                                                 */		
	...
	temp = (uint64_t)axi_data->src_info[VFE_PIX_0].pixel_clock;
//	pr_err("%s:temp= 0x%llu \n", __func__,temp);

		total_pix_bandwidth = total_pix_bandwidth /
			num_pix_streams * (num_pix_streams - 1) +
			(uint32_t)(temp *	ISP_DEFAULT_FORMAT_FACTOR / ISP_Q2);
	}
	total_bandwidth = total_pix_bandwidth + total_rdi_bandwidth;
//	pr_err("%s:total_bandwidth= 0x%llu total_pix_bandwidth=0x%x\n", __func__,total_bandwidth,total_pix_bandwidth);
#else
		total_pix_bandwidth = total_pix_bandwidth /
			num_pix_streams * (num_pix_streams - 1) +
			axi_data->src_info[VFE_PIX_0].pixel_clock *
	...
	total_bandwidth = total_pix_bandwidth + total_rdi_bandwidth;
	pr_err("%s:temp= 0x%lu \n", __func__,axi_data->src_info[VFE_PIX_0].pixel_clock);
	pr_err("%s:total_bandwidth= 0x%llu total_pix_bandwidth=0x%x\n", __func__,total_bandwidth,total_pix_bandwidth);
#endif
/*                                                                                                                */		
  ...
  struct msm_vfe_axi_halt_cmd *halt_cmd)
{
	...
	if (halt_cmd->stop_camif) {
		vfe_dev->hw_info->vfe_ops.core_ops.
			update_camif_state(vfe_dev, DISABLE_OVERFLOW);
	}
