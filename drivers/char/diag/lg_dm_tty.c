...
static int lge_dm_tty_read_thread(void *data)
{
	...
	int num_hsic_buf_tbl_entries_check = FALSE;
	...
	while (1) {
		...
		while ((lge_dm_tty->set_logging == 1)
...
#else
			for (index = 0; index < MAX_HSIC_DATA_CH; index++) {
				if(diag_hsic[index].num_hsic_buf_tbl_entries == N_MDM_WRITE) {
					num_hsic_buf_tbl_entries_check = TRUE;
					break;
				}
			...
			if(num_hsic_buf_tbl_entries_check == TRUE) {
				lge_dm_tty->set_logging = 1;
				num_hsic_buf_tbl_entries_check = FALSE;
			}
#endif /* CONFIG_DIAGFWD_BRIDGE_CODE */
