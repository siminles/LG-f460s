...
#define MAX_SPI_BUFFER_SIZE (NUM_ACTUATORS * \
...
static long unlocked_ioctl(struct file *file,
...
static ssize_t
immersion_enable_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t
immersion_enable_show(struct device *dev,
...
static struct device_attribute immersion_device_attrs[] = {
	__ATTR(vib_enable, S_IRUGO | S_IWUSR,
			immersion_enable_show, immersion_enable_store),
};
...
static int __init tspdrv_init(void)
{
	...
	DbgOut((DBL_ERROR, "*** tspdrv: runtime recorder feature is ON for debug"
				"which should be OFF in release version.\n"
				" *** tspdrv: please turn off the feature "
				"by removing VIBE_RUNTIME_RECODE macro.\n"));
#endif
	...
	if (nRet)
	{
		DbgOut((DBL_ERROR, "tspdrv: platform_device_reg failed.\n"));
	}

	nRet = platform_driver_register(&platdrv);
	if (nRet)
	{
		DbgOut((DBL_ERROR, "tspdrv: platform_driver_reg failed.\n"));
	}
#if 1
	/*                  
  ...
  */
	for (i = 0; i < ARRAY_SIZE(immersion_device_attrs); i++) {
		nRet = device_create_file(miscdev.this_device,
				&immersion_device_attrs[i]);
		if (nRet)
			return nRet;
	}
		...
		const char *buf, size_t count, loff_t *ppos)
{
    ...
    */
    ...
    /*
    ** Ignore packets that have size smaller than SPI_HEADER_SIZE
       or bigger than MAX_SPI_BUFFER_SIZE.
    ** Please note that the daemon may send an empty buffer
       (count == SPI_HEADER_SIZE)
    ** during quiet time between effects while playing a Timeline effect
       in order to maintain
    ** correct timing: if "count" is equal to SPI_HEADER_SIZE,
       the call to VibeOSKernelLinuxStartTimer()
		...
		case TSPDRV_SET_DBG_LEVEL:
			{
							...
							sizeof(long)))
				{
					...
					DbgOut((DBL_ERROR, "copy_from_user "
								"failed "
								"to copy debug "
								"lvl data.\n"));
					return -1;
				...
				if (DBL_TEMP <= nDbgLevel &&
						nDbgLevel <= DBL_OVERKILL)
				{
					atomic_set(&g_nDebugLevel, nDbgLevel);
				} else {
					DbgOut((DBL_ERROR, "Invalid debug level "
								"requested, ignored."));
				}
