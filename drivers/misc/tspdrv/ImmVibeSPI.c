 ...
 **
 ** Portions Copyright (c) 2008-2010 Immersion Corporation.
 All Rights Reserved.
 **
...
/* extern int qpnp_vib_set_with_vtglevel(struct qpnp_vib *vib,
int vtglevel, int on); */
...
#define MMSS_CC_PWM_SET		0xFC401984
...
static int sm100_power_set(int enable, struct timed_vibrator_data *vib_data)
{
	...
	if (vib_data->vpwr_on != 1) {
		if (enable) {
			rc = regulator_enable(vib_data->vreg_l19);
			if (rc < 0)
				ERR_MSG("regulator_enable failed\n");
		} else {
			if (regulator_is_enabled(vib_data->vreg_l19) > 0) {
				rc = regulator_disable(vib_data->vreg_l19);
				if (rc < 0)
...
#ifdef CONFIG_OF
static void sm100_parse_dt(struct device *dev,
		struct timed_vibrator_data *vib_data)
{
	...
	vib_data->haptic_en_gpio = of_get_named_gpio_flags(np,
			"syncoam,haptic-pwr-gpio", 0, NULL);
	vib_data->motor_pwm_gpio = of_get_named_gpio_flags(np,
			"syncoam,motor-pwm-gpio", 0, NULL);

	of_property_read_u32(np, "syncoam,n-value", &vib_data->vibe_n_value);
	of_property_read_u32(np, "syncoam,clk-rate", &vib_data->clk_rate);

	INFO_MSG("vpwr_on:%d, en_gpio:%d, pwm_gpio:%d, "
			"vibe_n_value:%d, clk_rate:%u\n",
			vib_data->vpwr_on,
...
static int sm100_probe(struct platform_device *pdev)
{
	...
	if (vib.vpwr_on != 1) {
		if (!(vib.vreg_l19)) {
			vib.vreg_l19 = regulator_get(&pdev->dev, "vdd_ana");
			if (IS_ERR(vib.vreg_l19)) {
				ERR_MSG("regulator_get failed (%ld)\n",
						PTR_ERR(vib.vreg_l19));
				vib.vreg_l19 = NULL;
				return 0;
			}
		}
	...
	gp3_clk = clk_get(&pdev->dev, "gp3_clk");
	clk_set_rate(gp3_clk, 29813);
		...
		VibeInt8* pForceOutputBuffer)
{
	...
	else
	{
		...
		if(sm100_flag) {
			...
			/* qpnp_vib_set_with_vtglevel(vib_dev,
			   (nForce * 31) / 128 + 1, true); */
#else
			/* qpnp_vib_set_with_vtglevel(vib_dev,
			   (nForce * 31) / 128 + 3, true); */
#endif
			}
		}
	}
