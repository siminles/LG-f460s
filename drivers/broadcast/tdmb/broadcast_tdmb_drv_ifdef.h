...
#define LGE_BROADCAST_TDMB_IOCTL_SET_CH \
	_IOW(LGE_BROADCAST_TDMB_IOCTL_MAGIC, 5, \
...
#define LGE_BROADCAST_TDMB_IOCTL_GET_SIG_INFO \
	_IOR(LGE_BROADCAST_TDMB_IOCTL_MAGIC, 8, \
			struct broadcast_tdmb_sig_info*)

#define LGE_BROADCAST_TDMB_IOCTL_GET_CH_INFO \
	_IOR(LGE_BROADCAST_TDMB_IOCTL_MAGIC, 9, \
...
#define LGE_BROADCAST_TDMB_IOCTL_GET_DMB_DATA \
	_IOW(LGE_BROADCAST_TDMB_IOCTL_MAGIC, 12, \
...
struct broadcast_drv_if {
	...
	int (*broadcast_drv_if_set_channel)(
		unsigned int freq_num, unsigned int subch_id, unsigned int op_mode);
	int (*broadcast_drv_if_detect_sync)(int op_mode);
	int (*broadcast_drv_if_get_sig_info)(
		struct broadcast_tdmb_sig_info *dmb_bb_info);
	int (*broadcast_drv_if_get_fic)(char* buffer, unsigned int* buffer_size);
	int (*broadcast_drv_if_get_msc)(
		char** buffer_ptr, unsigned int* buffer_size, unsigned int user_buffer_size);
