...
static int broadcast_tdmb_set_channel(void __user *arg)
{
	...
	if (copy_from_user(&udata, arg, sizeof(struct broadcast_tdmb_set_ch_info))) {
		...
		rc = device_drv->broadcast_drv_if_set_channel(udata.ch_num,
				udata.sub_ch_id, udata.mode);
	}
	return rc;
}
...
static int broadcast_tdmb_get_ch_info(void __user *arg)
{
	...
	struct broadcast_tdmb_ch_info __user *puserdata =
		(struct broadcast_tdmb_ch_info __user *)arg;
...
static int broadcast_tdmb_get_dmb_data(void __user *arg)
{
	...
	struct broadcast_tdmb_data_info __user *puserdata =
		(struct broadcast_tdmb_data_info  __user *)arg;
...
static ssize_t
	broadcast_tdmb_open_control(struct inode *inode, struct file *file)
{
...
static long broadcast_tdmb_ioctl_control
...
static ssize_t broadcast_tdmb_release_control(
...
static int broadcast_tdmb_device_init(
	struct broadcast_tdmb_chdevice *pbroadcast, int index)
{
	int rc;
