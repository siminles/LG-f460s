 ...
 *
 * $Id: linux_osl.c 468402 2014-04-07 19:26:20Z $
 */
...
typedef struct bcm_memory_alloc {
        unsigned int ptr;
        int size;
} bcm_memory_alloc_t;
#define MAX_BCM_MEMORY 8192
int memory_last=0;
int pre_free_loc = 0;

static bcm_memory_alloc_t BCM_MEMORY[MAX_BCM_MEMORY]={{0,0},};
int bcm_total_mem = 0;
int get_bcmtotal_mem(void){
        return bcm_total_mem;
}

int get_next_loc(void){
        int i=0;

        if (pre_free_loc != 0 )
                return pre_free_loc;

        for(i=0;i<MAX_BCM_MEMORY;++i){
                if (BCM_MEMORY[i].ptr == 0 )
                        break;
        }

        return i;
}
int bcm_find_memory(unsigned int ptr){
        int i=0;
        for(i=0;i<MAX_BCM_MEMORY;++i){
                if (BCM_MEMORY[i].ptr == ptr )
                        break;
        }
        return i;
}

void bcm_add_memory(unsigned int ptr, int size){
        int loc = get_next_loc();

        BCM_MEMORY[loc].ptr = ptr;
        BCM_MEMORY[loc].size = size;
        bcm_total_mem += size;
        pre_free_loc = 0;
        printk("BCMDHD_ALLOC[%d]:ptr = 0x%x, size = %d, total = %d \n",loc,ptr,size, bcm_total_mem);
}
void bcm_free_memory(int ptr){
        int found = bcm_find_memory(ptr);

        bcm_total_mem -= BCM_MEMORY[found].size;

        printk("BCMDHD_FREE[%d]:ptr = 0x%x, size =  %d , total = %d\n",found, BCM_MEMORY[found].ptr,BCM_MEMORY[found].size, bcm_total_mem);
        BCM_MEMORY[found].ptr = 0;
        BCM_MEMORY[found].size=0;
        pre_free_loc = found;
...
original:
	...
	if (osh && osh->cmn)
		atomic_add(size, &osh->cmn->malloced);
#if defined(MEMORY_DEBUG)
        bcm_add_memory((unsigned int)addr,(int)size);
#endif
...
#if defined(MEMORY_DEBUG)
		bcm_free_memory((unsigned int)addr);
#endif
	kfree(addr);
}
