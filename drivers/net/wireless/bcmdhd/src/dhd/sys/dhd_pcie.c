 ...
 *
 * $Id: dhd_pcie.c 471521 2014-04-21 01:52:47Z $
 */
...
void dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->sih->buscorerev == 6) || (bus->sih->buscorerev == 4) ||
		...
		status = si_corereg(bus->sih, bus->sih->buscoreidx, PCIMailBoxInt, 0, 0);
		status |= (status & bus->def_intmask);
		si_corereg(bus->sih, bus->sih->buscoreidx, PCIMailBoxInt, status, status);
	}
...
dhdpcie_bus_suspend(struct  dhd_bus *bus, bool state)
{
	...
	DHD_TRACE(("%s Enter with state :%d\n", __FUNCTION__, state));
	...
	if (state) {
		...
		if (bus->wait_for_d3_ack) {
			...
			if (dhd_os_check_wakelock_all(bus->dhd)) {
				...
				if (!rc) {
					DHD_ERROR(("%s:pci_set_power_state failed:current_state[%d]:ret[%d]\n", __func__, bus->dev->current_state, rc));
				}
				bus->suspended = FALSE;
...
dhd_update_txflowrings(dhd_pub_t *dhd)
{
	...
	for (flowid = 0; flowid < dhd->num_flow_rings; flowid++) {
...
void dhdpcie_l1ss_set(osl_t *osh)
{
	...
	OSL_PCI_WRITE_CONFIG(osh, 0x24c, 4, 0x29);
	OSL_PCI_WRITE_CONFIG(osh, 0x248, 4, 0xa0f);
	OSL_PCI_WRITE_CONFIG(osh, 0xbc, 4, 0x143);
	OSL_PCI_WRITE_CONFIG(osh, 0x1b4, 4, 0x10031003);
	OSL_PCI_WRITE_CONFIG(osh, 0xd4, 4, 0);
	OSL_PCI_WRITE_CONFIG(osh, 0xd4, 4, 0x400);

#ifdef SUPPORT_PME_USE
	OSL_PCI_WRITE_CONFIG(osh, 0x4c, 4, 0xc108); // for pme pin
#endif
}
#endif
