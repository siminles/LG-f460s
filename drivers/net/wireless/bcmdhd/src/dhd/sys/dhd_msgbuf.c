 ...
 *
 * $Id: dhd_msgbuf.c 470642 2014-04-15 23:29:11Z $
 */
#include <typedefs.h>
#include <osl.h>
...
#if defined(DHD_PCIE_CHECKSUM_ONLY_32BIT_MACHINE)
	uint32      epoch;
#endif
} msgbuf_ring_t;
...
typedef struct dhd_prot {
	...
	void*		d2h_dma_scratch_buf;	/* For holding ioct request buf */
	dmaaddr_t	d2h_dma_scratch_buf_phys; /* physical address for ioctbuf */

	uint32	h2d_dma_writeindx_buf_len; /* For holding dma ringupd buf - submission write */
	void*	h2d_dma_writeindx_buf;	/* For holding dma ringupd buf - submission write */
	dmaaddr_t h2d_dma_writeindx_buf_phys; /* physical address for dma ringupd - subm. write */

	uint32	h2d_dma_readindx_buf_len; /* For holding dma ringupd buf - submission read */
	void*	h2d_dma_readindx_buf;	/* For holding dma ringupd buf - submission read */
	dmaaddr_t h2d_dma_readindx_buf_phys; /* physical address for dma ringupd - subm. read */

	uint32	d2h_dma_writeindx_buf_len; /* For holding dma ringupd buf - completion write */
	void*	d2h_dma_writeindx_buf;	/* For holding dma ringupd buf - completion write */
	dmaaddr_t d2h_dma_writeindx_buf_phys; /* physical address for dma ringupd - compl. write */

	uint32	d2h_dma_readindx_buf_len; /* For holding dma ringupd buf - completion read */
	void*	d2h_dma_readindx_buf;	/* For holding dma ringupd buf - completion read */
	dmaaddr_t d2h_dma_readindx_buf_phys; /* physical address for dma ringupd - compl. read */
...
#if defined(DHD_PCIE_CHECKSUM_ONLY_32BIT_MACHINE) // if 32bit host please enable this
#define PCIE_M2M_D2H_XOR_CSUM
#define PCIE_M2M_HOST_READ_BARRIER
...
#endif /* BCM_ROUTER_DHD: 32bit Hosts */
...
static void
dhd_prhex(const char *msg, uchar *buf, uint nbytes)
{
	char line[128], *p;
	int len = sizeof(line);
	int nchar;
                           ...
                           cmn_msg_hdr_t *msg, int msglen)
{
	...
	for (tries = 0; (tries < PCIE_M2M_D2H_DMA_WAIT_TRIES); tries++) {
		...
		uint32 msg_checksum = *marker; /* read the marker */
		prot_checksum = bcm_compute_xor32((uint32*)msg, num_words);

		if ((prot_checksum ^ msg_checksum) == msg_checksum) { /* checksum OK */
		...
		DHD_INFO(("======== DHD<%p> try<%u> msg<0x%08x:%u> != prot<0x%08x:%u>\n",
			dhd, tries, msg_checksum, msg->epoch, prot_checksum,
	...
	DHD_ERROR(("LIVELOCK DHD<%p> tries<%u> msg<0x%08x:%u> != prot<0x%08x:%u>\n",
		dhd, tries, *marker, msg->epoch, prot_checksum,
		(ring->epoch % D2H_EPOCH_MODULO)));

	dhd_prhex("Rx MsgBuf Failure", (uchar *)msg, msglen);
...
int dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	prot->retbuf = DMA_ALLOC_CONSISTENT(dhd->osh, IOCT_RETBUF_SIZE, DMA_ALIGN_LEN,
		&alloced, &prot->retbuf_phys, NULL);
	if (prot->retbuf ==  NULL) {
		ASSERT(0);
		return BCME_NOMEM;
	}

	ASSERT(MODX((unsigned long)prot->retbuf, DMA_ALIGN_LEN) == 0);
	bzero(prot->retbuf, IOCT_RETBUF_SIZE);
	OSL_CACHE_FLUSH((void *) prot->retbuf, IOCT_RETBUF_SIZE);

	/* IOCTL request buffer */
	prot->ioctbuf = DMA_ALLOC_CONSISTENT(dhd->osh, IOCT_RETBUF_SIZE, DMA_ALIGN_LEN,
		&alloced, &prot->ioctbuf_phys, NULL);

	if (prot->ioctbuf ==  NULL) {
		ASSERT(0);
		return BCME_NOMEM;
	}

	ASSERT(MODX((unsigned long)prot->ioctbuf, DMA_ALIGN_LEN) == 0);
	bzero(prot->ioctbuf, IOCT_RETBUF_SIZE);
	OSL_CACHE_FLUSH((void *) prot->ioctbuf, IOCT_RETBUF_SIZE);

	/* Scratch buffer for dma rx offset */
	prot->d2h_dma_scratch_buf_len = DMA_D2H_SCRATCH_BUF_LEN;
	prot->d2h_dma_scratch_buf = DMA_ALLOC_CONSISTENT(dhd->osh, 
#if defined(PCIE_M2M_HOST_READ_BARRIER)
		sizeof(dhd_read_barrier_t) +
#endif
		DMA_D2H_SCRATCH_BUF_LEN,
		DMA_ALIGN_LEN, &alloced, &prot->d2h_dma_scratch_buf_phys, NULL);

	if (prot->d2h_dma_scratch_buf == NULL) {
		ASSERT(0);
		return BCME_NOMEM;
	}
	ASSERT(MODX((unsigned long)prot->d2h_dma_scratch_buf, DMA_ALIGN_LEN) == 0);
	bzero(prot->d2h_dma_scratch_buf,
#if defined(PCIE_M2M_HOST_READ_BARRIER)
		sizeof(dhd_read_barrier_t) +
#endif
...
#if defined(PCIE_M2M_HOST_READ_BARRIER)
	{
		...
		rb = (dhd_read_barrier_t *)(prot->d2h_dma_scratch_buf
			+ DMA_D2H_SCRATCH_BUF_LEN);
...
dhd_prot_init_index_dma_block(dhd_pub_t *dhd, uint8 type, uint32 length)
{
	...
	switch (type) {
		case HOST_TO_DNGL_DMA_WRITEINDX_BUFFER:
			...
			prot->h2d_dma_writeindx_buf = DMA_ALLOC_CONSISTENT(dhd->osh,
				dma_block_size, DMA_ALIGN_LEN, &alloced,
				&prot->h2d_dma_writeindx_buf_phys, NULL);

			if (prot->h2d_dma_writeindx_buf == NULL) {
				return BCME_NOMEM;
			}

			ASSERT(ISALIGNED(prot->h2d_dma_writeindx_buf, 4));
			bzero(prot->h2d_dma_writeindx_buf, dma_block_size);
			OSL_CACHE_FLUSH((void *)prot->h2d_dma_writeindx_buf, dma_block_size);
		...
		case HOST_TO_DNGL_DMA_READINDX_BUFFER:
			...
			prot->h2d_dma_readindx_buf = DMA_ALLOC_CONSISTENT(dhd->osh,
				dma_block_size, DMA_ALIGN_LEN, &alloced,
				&prot->h2d_dma_readindx_buf_phys, NULL);
			if (prot->h2d_dma_readindx_buf == NULL) {
				return BCME_NOMEM;
			}

			ASSERT(ISALIGNED(prot->h2d_dma_readindx_buf, 4));
			bzero(prot->h2d_dma_readindx_buf, dma_block_size);
			OSL_CACHE_FLUSH((void *)prot->h2d_dma_readindx_buf, dma_block_size);
		...
		case DNGL_TO_HOST_DMA_WRITEINDX_BUFFER:
			...
			prot->d2h_dma_writeindx_buf = DMA_ALLOC_CONSISTENT(dhd->osh,
				dma_block_size, DMA_ALIGN_LEN, &alloced,
				&prot->d2h_dma_writeindx_buf_phys, NULL);

			if (prot->d2h_dma_writeindx_buf == NULL) {
				return BCME_NOMEM;
			}

			ASSERT(ISALIGNED(prot->d2h_dma_writeindx_buf, 4));
			bzero(prot->d2h_dma_writeindx_buf, dma_block_size);
			OSL_CACHE_FLUSH((void *)prot->d2h_dma_writeindx_buf, dma_block_size);
		...
		case DNGL_TO_HOST_DMA_READINDX_BUFFER:
			...
			prot->d2h_dma_readindx_buf = DMA_ALLOC_CONSISTENT(dhd->osh,
				dma_block_size, DMA_ALIGN_LEN, &alloced,
				&prot->d2h_dma_readindx_buf_phys, NULL);

			if (prot->d2h_dma_readindx_buf == NULL) {
				return BCME_NOMEM;
			}

			ASSERT(ISALIGNED(prot->d2h_dma_readindx_buf, 4));
			bzero(prot->d2h_dma_readindx_buf, dma_block_size);
			OSL_CACHE_FLUSH((void *)prot->d2h_dma_readindx_buf, dma_block_size);
...
void dhd_prot_detach(dhd_pub_t *dhd)
{
	...
	if (dhd->prot) {
		...
		if (prot->d2h_dma_scratch_buf) {
			DMA_FREE_CONSISTENT(dhd->osh, prot->d2h_dma_scratch_buf,
#if defined(PCIE_M2M_HOST_READ_BARRIER)
				sizeof(dhd_read_barrier_t) +
#endif
				DMA_D2H_SCRATCH_BUF_LEN, prot->d2h_dma_scratch_buf_phys, NULL);
			prot->d2h_dma_scratch_buf = NULL;
		}
		/* free up ring upd buffer for submission writes */
		if (prot->h2d_dma_writeindx_buf) {
			DMA_FREE_CONSISTENT(dhd->osh, prot->h2d_dma_writeindx_buf,
			  prot->h2d_dma_writeindx_buf_len, prot->h2d_dma_writeindx_buf_phys, NULL);
			prot->h2d_dma_writeindx_buf = NULL;
		}

		/* free up ring upd buffer for submission reads */
		if (prot->h2d_dma_readindx_buf) {
			DMA_FREE_CONSISTENT(dhd->osh, prot->h2d_dma_readindx_buf,
			  prot->h2d_dma_readindx_buf_len, prot->h2d_dma_readindx_buf_phys, NULL);
			prot->h2d_dma_readindx_buf = NULL;
		}

		/* free up ring upd buffer for completion writes */
		if (prot->d2h_dma_writeindx_buf) {
			DMA_FREE_CONSISTENT(dhd->osh, prot->d2h_dma_writeindx_buf,
			  prot->d2h_dma_writeindx_buf_len, prot->d2h_dma_writeindx_buf_phys, NULL);
			prot->d2h_dma_writeindx_buf = NULL;
		}

		/* free up ring upd buffer for completion writes */
		if (prot->d2h_dma_readindx_buf) {
			DMA_FREE_CONSISTENT(dhd->osh, prot->d2h_dma_readindx_buf,
			  prot->d2h_dma_readindx_buf_len, prot->d2h_dma_readindx_buf_phys, NULL);
			prot->d2h_dma_readindx_buf = NULL;
		}
		...
		if (prot->ioctbuf) {
			...
			IOCT_RETBUF_SIZE, dhd->prot->ioctbuf_phys, NULL);
...
int dhd_prot_init(dhd_pub_t *dhd)
{
	...
	dhd_bus_cmn_writeshared(dhd->bus, &prot->d2h_dma_scratch_buf_phys,
		sizeof(prot->d2h_dma_scratch_buf_phys), DNGL_TO_HOST_DMA_SCRATCH_BUFFER, 0);
	...
	if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support)) {
		dhd_bus_cmn_writeshared(dhd->bus, &prot->d2h_dma_writeindx_buf_phys,
			sizeof(prot->d2h_dma_writeindx_buf_phys),
			DNGL_TO_HOST_DMA_WRITEINDX_BUFFER, 0);
		dhd_bus_cmn_writeshared(dhd->bus, &prot->h2d_dma_readindx_buf_phys,
			sizeof(prot->h2d_dma_readindx_buf_phys),
			HOST_TO_DNGL_DMA_READINDX_BUFFER, 0);
	}

	if (DMA_INDX_ENAB(dhd->dma_h2d_ring_upd_support)) {
		dhd_bus_cmn_writeshared(dhd->bus, &prot->h2d_dma_writeindx_buf_phys,
			sizeof(prot->h2d_dma_writeindx_buf_phys),
			HOST_TO_DNGL_DMA_WRITEINDX_BUFFER, 0);
		dhd_bus_cmn_writeshared(dhd->bus, &prot->d2h_dma_readindx_buf_phys,
			sizeof(prot->d2h_dma_readindx_buf_phys),
...
#ifdef CUSTOMER_HW10
		/* Update read pointer as soon as possible in case of CUSTOMER_HW10 */
		/* Late update induces firmware stuck on APQ8084 */
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	DHD_TRACE(("posting a Tx packet to dongle pktlen %d\n", pktlen));
	...
	if (txdesc == NULL) {
		...
		/* Free up the PKTID and unmap the DMA buffer */
		PKTID_TO_NATIVE(dhd->prot->pktid_map_handle, pktid, physaddr, pktlen);
		DHD_GENERAL_UNLOCK(dhd, flags);
	...
	if (prot->tx_metadata_offset && (headroom >= prot->tx_metadata_offset)) {
		...
		meta_physaddr = DMA_MAP(dhd->osh, pktdata - prot->tx_metadata_offset,
			prot->tx_metadata_offset, DMA_RX, 0, 0);
		if ((PHYSADDRHI(meta_physaddr) == 0) && (PHYSADDRLO(meta_physaddr) == 0)) {
			DHD_ERROR(("Something really bad, unless 0 is a valid phyaddr\n"));
			ASSERT(0);
		}
...
int dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf, int len)
{
	...
	if ((!ret) && (ioc->cmd == WLC_SET_VAR) && (!strcmp(buf, "awdl_peer_op"))) {
		int slen;
		slen = strlen("awdl_peer_op") + 1;
		dhd_awdl_peer_delete(dhd, (uint8)ifidx, ((char *)buf + slen), len - slen);
	}
	/* Intercept the awdl ioctl here, delete flow rings if awdl is disabled */
	if ((!ret) && (ioc->cmd == WLC_SET_VAR) && (!strcmp(buf, "awdl"))) {
		int slen, val = 0;
		slen = strlen("awdl") + 1;
		if (len >= (int)(slen + sizeof(int))) {
			bcopy(((char *)buf + slen), &val, sizeof(int));
			val = ltoh32(val);
			if (!val)
				dhd_awdl_peer_delete(dhd, (uint8)ifidx, NULL, 0);
		}
	}
...
dhd_fillup_ioct_reqst_ptrbased(dhd_pub_t *dhd, uint16 len, uint cmd, void* buf, int ifidx)
{
	...
	ioct_rqst->host_input_buf_addr.high = htol32(PHYSADDRHI(prot->ioctbuf_phys));
	ioct_rqst->host_input_buf_addr.low = htol32(PHYSADDRLO(prot->ioctbuf_phys));
	/* copy ioct payload */
	ioct_buf = (void *) prot->ioctbuf;

	if (buf)
		memcpy(ioct_buf, buf, len);

	OSL_CACHE_FLUSH((void *) prot->ioctbuf, len);
...
prot_ring_attach(dhd_prot_t * prot, char* name, uint16 max_item, uint16 len_item, uint16 ringid)
{
	...
	ring->ring_base = DMA_ALLOC_CONSISTENT(prot->osh, size, DMA_ALIGN_LEN,
		&alloced, &physaddr, NULL);

	if (ring->ring_base ==  NULL)
		goto fail;
	ring->ringmem->base_addr.high_addr = htol32(PHYSADDRHI(physaddr));
	ring->ringmem->base_addr.low_addr = htol32(PHYSADDRLO(physaddr));

	ASSERT(MODX((unsigned long)ring->ring_base, DMA_ALIGN_LEN) == 0);
	...
	DHD_INFO(("RING_ATTACH : %s Max item %d len item %d total size %d "
		...
		size, ring->ring_base, ring->ringmem->base_addr.high_addr,
...
fail:
	if (ring->ring_base)
		...
		DMA_FREE_CONSISTENT(prot->osh, ring->ring_base, size, physaddr, NULL);
		ring->ring_base = NULL;
	if (ring->ringmem)
...
dhd_prot_ring_detach(dhd_pub_t *dhd, msgbuf_ring_t * ring)
{
	...
	/* Free up ring */
	if (ring->ring_base) {
		DMA_FREE_CONSISTENT(prot->osh, ring->ring_base, size, phyaddr, NULL);
		ring->ring_base = NULL;
	}
...
dhd_set_dmaed_index(dhd_pub_t *dhd, uint8 type, uint16 ringid, uint16 new_index)
{
	...
	switch (type) {
		case H2D_DMA_WRITEINDX:
			ptr = (uint32 *)(prot->h2d_dma_writeindx_buf);
			...
			*ptr = htol16(new_index);
			...
			/* cache flush */
			OSL_CACHE_FLUSH((void *)prot->h2d_dma_writeindx_buf,
				prot->h2d_dma_writeindx_buf_len);

			break;

		case D2H_DMA_READINDX:
			ptr = (uint32 *)(prot->d2h_dma_readindx_buf);
			...
			*ptr = htol16(new_index);
			/* cache flush */
			OSL_CACHE_FLUSH((void *)prot->d2h_dma_readindx_buf,
				prot->d2h_dma_readindx_buf_len);
...
dhd_get_dmaed_index(dhd_pub_t *dhd, uint8 type, uint16 ringid)
{
	...
	switch (type) {
		case H2D_DMA_WRITEINDX:
			OSL_CACHE_INV((void *)dhd->prot->h2d_dma_writeindx_buf,
				dhd->prot->h2d_dma_writeindx_buf_len);
			ptr = (uint32 *)(dhd->prot->h2d_dma_writeindx_buf);
		...
		case H2D_DMA_READINDX:
			OSL_CACHE_INV((void *)dhd->prot->h2d_dma_readindx_buf,
				dhd->prot->h2d_dma_readindx_buf_len);
			ptr = (uint32 *)(dhd->prot->h2d_dma_readindx_buf);
		...
		case D2H_DMA_WRITEINDX:
			OSL_CACHE_INV((void *)dhd->prot->d2h_dma_writeindx_buf,
				dhd->prot->d2h_dma_writeindx_buf_len);
			ptr = (uint32 *)(dhd->prot->d2h_dma_writeindx_buf);
		...
		case D2H_DMA_READINDX:
			OSL_CACHE_INV((void *)dhd->prot->d2h_dma_readindx_buf,
				dhd->prot->d2h_dma_readindx_buf_len);
			ptr = (uint32 *)(dhd->prot->d2h_dma_readindx_buf);
...
prot_get_src_addr(dhd_pub_t *dhd, msgbuf_ring_t * ring, uint16* available_len)
{
	...
	DHD_TRACE(("%s: h2d_dma_readindx_buf %p, d2h_dma_writeindx_buf %p\n",
		__FUNCTION__, (uint32 *)(dhd->prot->h2d_dma_readindx_buf),
		(uint32 *)(dhd->prot->d2h_dma_writeindx_buf)));
	...
	*available_len = READ_AVAIL_SPACE(w_ptr, r_ptr, depth);
	...
	ret_addr = (char*)ring->ring_base + (r_ptr * ring->ringmem->len_items);
...
dhd_prot_ringupd_dump(dhd_pub_t *dhd, struct bcmstrbuf *b)
{
	...
	OSL_CACHE_INV((void *)dhd->prot->h2d_dma_readindx_buf, dhd->prot->h2d_dma_readindx_buf_len);
...
dhd_prot_clear(dhd_pub_t *dhd)
{
	...
	if(prot->retbuf) {
		OSL_CACHE_INV((void *) prot->retbuf, IOCT_RETBUF_SIZE);
		bzero(prot->retbuf, IOCT_RETBUF_SIZE);
		OSL_CACHE_FLUSH((void *) prot->retbuf, IOCT_RETBUF_SIZE);
	}

	if(prot->ioctbuf) {
		OSL_CACHE_INV((void *) prot->ioctbuf, IOCT_RETBUF_SIZE);
		bzero(prot->ioctbuf, IOCT_RETBUF_SIZE);
		OSL_CACHE_FLUSH((void *) prot->ioctbuf, IOCT_RETBUF_SIZE);	
	}

	if(prot->d2h_dma_scratch_buf) {
		OSL_CACHE_INV((void *)prot->d2h_dma_scratch_buf,
#if defined(PCIE_M2M_HOST_READ_BARRIER)
				sizeof(dhd_read_barrier_t) +
#endif
				DMA_D2H_SCRATCH_BUF_LEN);
	...
	if(prot->h2d_dma_readindx_buf) {
		OSL_CACHE_INV((void *)prot->h2d_dma_readindx_buf, prot->h2d_dma_readindx_buf_len);	
		bzero(prot->h2d_dma_readindx_buf, prot->h2d_dma_readindx_buf_len);
		OSL_CACHE_FLUSH((void *)prot->h2d_dma_readindx_buf, prot->h2d_dma_readindx_buf_len);
	}

	if(prot->h2d_dma_writeindx_buf) {
		OSL_CACHE_INV((void *)prot->h2d_dma_writeindx_buf, prot->h2d_dma_writeindx_buf_len);	
		bzero(prot->h2d_dma_writeindx_buf, prot->h2d_dma_writeindx_buf_len);
		OSL_CACHE_FLUSH((void *)prot->h2d_dma_writeindx_buf, prot->h2d_dma_writeindx_buf_len);
	}

	if(prot->d2h_dma_readindx_buf) {
		OSL_CACHE_INV((void *)prot->d2h_dma_readindx_buf, prot->d2h_dma_readindx_buf_len);	
		bzero(prot->d2h_dma_readindx_buf, prot->d2h_dma_readindx_buf_len);
		OSL_CACHE_FLUSH((void *)prot->d2h_dma_readindx_buf, prot->d2h_dma_readindx_buf_len);
	}

	if(prot->d2h_dma_writeindx_buf) {
		OSL_CACHE_INV((void *)prot->d2h_dma_writeindx_buf, prot->d2h_dma_writeindx_buf_len);	
		bzero(prot->d2h_dma_writeindx_buf, prot->d2h_dma_writeindx_buf_len);
		OSL_CACHE_FLUSH((void *)prot->d2h_dma_writeindx_buf, prot->d2h_dma_writeindx_buf_len);
	}
