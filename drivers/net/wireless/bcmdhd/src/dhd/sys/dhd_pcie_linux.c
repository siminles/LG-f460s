 ...
 *
 * $Id: dhd_pcie_linux.c 467622 2014-04-03 20:48:44Z $
 */
...
static int dhdpcie_set_suspend_resume(struct pci_dev *pdev, bool state)
{
	...
	DHD_ERROR(("%s Enter with 07 state :%x otp=%d\n", __FUNCTION__, state, pdev->device));

	if (pch)
		bus = pch->bus;

	if (bus && bus->dhd && bus->dhd->prot && ((bus->dhd->busstate == DHD_BUS_DATA) || (bus->dhd->busstate == DHD_BUS_SUSPEND)) &&
		(bus->suspended != state)) {
...
{
	BCM_REFERENCE(state);
	DHD_TRACE(("%s Enter with event %x\n", __FUNCTION__, state.event));
