 ...
 *
 * $Id: dhd_linux.c 470166 2014-04-14 10:13:48Z $
 */
...
#ifdef BCMPCIE		/* ASYNC_INIT feature can be used with PCIE driver */
#define ASYNC_INIT	/* use async_schedule to reduce boot-up time */
#endif
#ifdef ASYNC_INIT
#include <linux/async.h>
#endif
#ifdef CHIP_REV_CHCEK_FOR_LGE
int linkdown_cnt = 0;
int hang_cnt = 0;
#endif
...
dhd_ifname2idx(dhd_info_t *dhd, char *name)
{
	...
	if (name == NULL || *name == '\0')
		return 0;
#ifdef CUSTOMER_HW10
	else if (!strcmp(name, "wl0"))
		strcpy(name, "wlan0");
	else if (!strcmp(name, "wl0.2"))
		strcpy(name, "p2p-wlan0-0");
#endif
...
dhd_ifadd_event_handler(void *handle, void *event_info, u8 event)
{
	...
	if (if_event->event.role != WLC_E_IF_ROLE_STA) {
...
dhd_sendpkt(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
	...
	if (!dhdp->up || (dhdp->busstate == DHD_BUS_DOWN)) {
		...
		DHD_ERROR(("%s : sendpkt failed , busstate = %d\n",__FUNCTION__, dhdp->busstate));
...
#if defined(BCMPCIE) && defined (CUSTOMER_HW10)
...
#ifdef CUSTOMER_HW10
	/* WAR for wake lock/unlock unbalance issue */
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	dhd->pub.osh = osh;
	dhd->adapter = adapter;
#if defined(CUSTOMER_HW10) && defined(BCMPCIE)
...
/* check wifi chip - bcm4354,bcm4356. for only T6 PRJ. moon_wifi*/
#ifdef CHIP_REV_CHCEK_FOR_LGE
	chiprev = dhd_bus_chiprev_id(&dhdinfo->pub);
	DHD_ERROR(("%s chiprev=%d dhdinfo=%p\n", __func__ , chiprev, dhdinfo));
	if (chiprev == 1) {
		if (firmware_path[0] != '\0') {
			if (strnstr(firmware_path, "_apsta", strlen(firmware_path)) != NULL)
				fw = "/system/etc/firmware/fw_bcmdhd_apsta_4354.bin";
			else if (strnstr(firmware_path, "_mfg", strlen(firmware_path)) != NULL)
				fw = "/system/etc/firmware/fw_bcmdhd_mfg_4354.bin";
			else
				fw = "/system/etc/firmware/fw_bcmdhd_4354.bin";
		}
		nv = "/system/etc/wifi/bcmdhd_4354.cal";
	} else {
#endif
		if (firmware_path[0] != '\0')
			fw = firmware_path;
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	extern void dhd_prot_d2h_epoch_init(struct dhd_prot * prot);
#endif
	ASSERT(dhd);
	...
	if  (dhd->pub.busstate == DHD_BUS_DOWN && dhd_update_fw_nv_path(dhd)) {
		...
		if (ret < 0) {
			...
			DHD_ERROR(("%s: Success downloading firmware \n", __func__));
	}
...
#if defined(DHD_PCIE_CHECKSUM_ONLY_32BIT_MACHINE)
	dhd_prot_d2h_epoch_init(dhdp->prot);
#endif
...
int dhd_set_ap_powersave(dhd_pub_t *dhdp, int ifidx, int enable)
{
	...
	if (enable) {
		...
		if (dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) != BCME_OK) {
		...
		bcm_mkiovar("rxchain_pwrsave_quiet_time", (char *)&quiet_time, 4, iovbuf, sizeof(iovbuf));
		if (dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) != BCME_OK) {
			DHD_ERROR(("Failed to set quiet time"));
		}
		bcm_mkiovar("rxchain_pwrsave_stas_assoc_check", (char *)&stas_assoc_check, 4, iovbuf, sizeof(iovbuf));
		if (dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) != BCME_OK) {
...
static void __init dhd_module_init_async(void *data, async_cookie_t cookie)
{
	...
	/* wait until pcie.c:pcie_init_async() is done */
