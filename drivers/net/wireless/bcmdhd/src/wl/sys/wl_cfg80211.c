 ...
 *
 * $Id: wl_cfg80211.c 470165 2014-04-14 10:05:38Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
wl_cfg80211_notify_ifadd(int ifidx, char *name, uint8 *mac, uint8 bssidx)
{
	...
	if (wl_get_p2p_status(cfg, IF_ADDING)) {
		ifadd_expected = TRUE;
		wl_clr_p2p_status(cfg, IF_ADDING);
	} else if (cfg->ibss_pending_op) {
		ifadd_expected = TRUE;
		cfg->ibss_pending_op = FALSE;
	}
...
bcm_cfg80211_add_ibss_if(struct wiphy *wiphy, char *name)
{
	...
	timeout = wait_event_interruptible_timeout(cfg->netif_change_event,
		!cfg->ibss_pending_op, msecs_to_jiffies(MAX_WAIT_TIME));
	if (timeout <= 0 || cfg->ibss_pending_op)
		goto fail;
		...
		if (!wl_cfgp2p_is_p2p_action(frame, frame_len)) {
			...
			for_each_bss(bss_list, bi, i) {
				if (CHSPEC_IS5G(bi->chanspec) &&
					(wf_chspec_ctlchan(bi->chanspec) == af_params->channel)) {
...
{
	s32 err = 0;
#ifdef CUSTOMER_HW10 // for softap ap_isolate
...
#ifdef CUSTOMER_HW10 // for softap ap_isolate
		if (ap_isolate == 0)
			ap_isolate = AP_ISOLATE_SENDUP_ALL;
		else
			ap_isolate = AP_ISOLATE_DISABLED;
#endif
...
#ifdef PNO_DEBUG_FOR_LGE
	WL_ERR((">>> SCHED SCAN START n_match_sets:%d n_ssids:%d \n", request->n_match_sets, request->n_ssids));
#endif
...
#ifdef PNO_DEBUG_FOR_LGE
		WL_ERR((">>> SCHED SCAN START cfg->sched_scan_req=%p \n", cfg->sched_scan_req));
#endif
...
#ifdef PNO_DEBUG_FOR_LGE
	WL_ERR((">>> SCHED SCAN STOP cfg->sched_scan_req=%p \n", cfg->sched_scan_req));
#endif
...
#ifdef PNO_DEBUG_FOR_LGE
		if (!aborted){
			WL_ERR((">>> SCHED SCAN 1 cfg80211_sched_scan_results cfg->sched_scan_req=%p\n", cfg->sched_scan_req));
...
#ifdef CUSTOMER_HW10
	static uint32 sleep_count = 0;
	uint32 deq_count = 0;
	uint32 etype = 0, prev_etype = 0;
...
#ifdef CUSTOMER_HW10
		curr_t = OSL_SYSUPTIME();
		if ((curr_t - old_t) < 50) {
			if (sleep_count++ && ((sleep_count % 100) == 0)) {
				WL_ERR(("P:%s:sleep_count[%d]:event[%d]:prev_event[%d]:deq_count[%d]\n", __FUNCTION__, sleep_count, etype, prev_etype, deq_count));
			}
		} else {
			sleep_count = 0;
		}
