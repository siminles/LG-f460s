...
static int32_t qpnp_vadc_clear_state(struct qpnp_vadc_chip *vadc)
{
	...
	if (rc < 0) {
		...
		if (!rc)
			pr_err("success to clear LATCHED INT %x\n", value);
		else
			pr_err("failed to clear LATCHED INT (rc = %d)", rc);
	}
...
#ifdef CONFIG_MACH_LGE
/* Register addresses list which is possible to be read,
   that related to PMA8084 VADC1_USR_VADC */
static int adc_reg[] = {0x04, 0x05, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13,
	0x15, 0x16, 0x18, 0x19, 0x1a, 0x1b, 0x40, 0x46,
	0x48, 0x50, 0x51, 0x52, 0x54, 0x55, 0x57, 0x59,
