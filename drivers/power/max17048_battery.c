...
#else
	/* Report 0.42%(0x300) ~ 1% to 1% */
	/* If Full and Empty is changed, need to modify the value, 3 */
	else if (batt_soc == 0 && buf[0] >= 3) {
		pr_err("%s : buf[0] is %d, upscale to 1%%\n"
				, __func__, buf[0]);
		batt_soc = 1;
...
int max17048_set_rcomp_by_temperature(struct i2c_client *client)
{
	...
	int temp;
	int rc;
#if !defined(CONFIG_SMB349_CHARGER) && !defined(CONFIG_CHARGER_BQ24296)
		...
		struct max17048_battery_model *mdata)
{
...
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
#ifndef CONFIG_LGE_PM_CUT_OFF_3_4V
	...
	pr_info("[MAX17048] platform data : "\
		"rcomp = %d, "\
		"temp_co_hot = %d, "\
		"temp_co_cold = %d, "\
			...
			const struct i2c_device_id *id)
{
	...
	if (smem_size != 0 && batt_id) {
		...
		_smem_batt_id = (*batt_id >>8) & 0x00ff; /* batt id -> HSB */
		pr_info("Battery was read in sbl is = %d\n", _smem_batt_id);
#else
		_smem_batt_id = *batt_id;
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
		 else if (_smem_batt_id == BATT_DS2704_L
			|| _smem_batt_id == BATT_ISL6296_C) {
			cell_info = LGC_LLL; /* LGC Battery */
		} else if (_smem_batt_id == BATT_DS2704_C
			|| _smem_batt_id == BATT_ISL6296_L) {
