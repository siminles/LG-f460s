...
#endif /* CONFIG_MAX17048_FUELGAUGE */
...
#ifdef CONFIG_LGE_PM_CHARGER_TEMP_SCENARIO
...
#ifdef CONFIG_LGE_PM_THERMALE_CHG_CONTROL_FOR_WLC
#define THERMALE_NOT_TRIGGER -1
#define THERMALE_ALL_CLEAR 0
#endif

#define BATT_TEMP_WAKE_LOCK_NAME		"batt_temp_wake_lock"
#define BATT_TEMP_OPERATION_HOLD_TIME	1000
...
#ifdef CONFIG_LGE_PM_THERMALE_CHG_CONTROL
	int chg_current_te;
#endif
#ifdef CONFIG_LGE_PM_THERMALE_CHG_CONTROL_FOR_WLC
...
#endif
	struct wake_lock batt_temp_wake_lock;
...
static int bq24296_get_usbin_adc(struct bq24296_chip *chip)
{
	...
	rc = qpnp_vadc_read(the_chip->vadc_dev, P_MUX5_1_3, &results);
	if (rc) {
		pr_err("Unable to read usbin adc rc=%d\n", rc);
		return NOT_INIT_VBUS_UV;
	} else {
		usb_vol = results.physical * 147; /* 1.47 = (R1+R2)/(R1)*100 */
		usb_vol = ROUND_USBIN(usb_vol);
		pr_info("DC_IN voltage: %d\n", usb_vol);
		return usb_vol;
	}
#else
...
static void bq24296_irq_worker(struct work_struct *work)
{
	...
	/* when battery removed, TI charger frequently send "charging done" interrupt in several ten times millisecond */
	if ((reg08 & CHRG_STAT_MASK) == CHRG_STAT_MASK) {
		if (!__bq24296_get_prop_batt_present(chip)) {
			bool charger = false;
...
static void bq24296_check_suspended_worker(struct work_struct *work)
{
	...
	if (chip->suspend || i2c_suspended) {
		...
		pr_debug("bq24296 resumed. chip->suspend:%d, i2c_suspended:%d\n", chip->suspend, i2c_suspended ? 1 : 0);
...
#if defined(CONFIG_CHARGER_UNIFIED_WLC)
			if (chip->wlc_present &&
				((sys_status & CHRG_STAT_MASK) == CHRG_STAT_MASK)) {
...
int bq24296_get_batt_temp_origin(void)
{
	...
	if (!wake_lock_active(&the_chip->batt_temp_wake_lock)) {
		wake_lock_timeout(&the_chip->batt_temp_wake_lock,
						msecs_to_jiffies(BATT_TEMP_OPERATION_HOLD_TIME));
	}

	rc = qpnp_vadc_read(the_chip->vadc_dev, LR_MUX6_AMUX_THM3, &results);

	if (wake_lock_active(&the_chip->batt_temp_wake_lock))
		wake_unlock(&the_chip->batt_temp_wake_lock);
...
static int bq24296_get_prop_batt_capacity(struct bq24296_chip *chip)
{
	...
	if(is_factory_cable() && !chip->batt_present) {
		pr_err("PIF+NOBATTERY Set 85 battery_level.\n");
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED:
		...
		break;
	case POWER_SUPPLY_PROP_SAFTETY_CHARGER_TIMER:
#if !defined(CONFIG_LGE_PM_FACTORY_SAFETY_TIMER_DISABLE)
...
static int
smpl_count_set(const char *val, struct kernel_param *kp)
{
	int ret;
...
static int __init bq24296_init_batt_psy(struct bq24296_chip *chip)
{
	int ret;
	NULL_CHECK(chip, -EINVAL);
...
static int __init bq24296_init_ac_psy(struct bq24296_chip *chip)
{
	int ret = 0;
	NULL_CHECK(chip, -EINVAL);
				  ...
				  const struct i2c_device_id *id)
{
...
#if defined(CONFIG_LGE_PM_BATTERY_ID_CHECKER)
	uint *smem_batt = 0;
#if defined(CONFIG_LGE_PM_LOW_BATT_LIMIT)
	uint _smem_batt_ = 0;
#endif
#endif
	unsigned int *p_cable_type = (unsigned int *)
	...
	wake_lock_init(&chip->batt_temp_wake_lock,
			       WAKE_LOCK_SUSPEND, BATT_TEMP_WAKE_LOCK_NAME);
	...
	if (!chip->watchdog) {
		ret = bq24296_masked_write(chip->client,
			BQ05_CHARGE_TERM_TIMER_CONT_REG, I2C_TIMER_MASK, 0x00);
		if (ret) {
...
err_init_batt_psy:
	...
	wake_lock_destroy(&chip->batt_temp_wake_lock);
