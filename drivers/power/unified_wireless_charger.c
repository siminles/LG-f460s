...
#elif defined(CONFIG_CHARGER_BQ24296)
...
int checking_insert_cnt;
int checking_usb_insert_cnt;
...
set_wlc_thermal_mitigation(const char *val, struct kernel_param *kp)
{
	...
	if (!(wlc_thermal_mitigation == THERMALE_ALL_CLEAR) &&
		!(wlc_thermal_mitigation >= IBAT_MIN_MA &&
		wlc_thermal_mitigation <= IBAT_MAX_MA)) {
...
static void wireless_set_online_work(struct work_struct *work)
{
	...
	if (wlc && vbus) {
		pr_debug("[WLC] %s : Wireless psy is updated to ONLINE !!!!!\n",
				__func__);
	...
	if (!wlc || (checking_insert_cnt > MAX_CHECKING_COUNT)) {
		pr_debug("[WLC] %s:It was Ghost WLC (wlc(%d) vbus(%d)).....count : %d\n"
			, __func__, wlc, vbus, checking_insert_cnt);
	} else {
		pr_debug("[WLC] %s:Checking Ghost WLC (wlc(%d) vbus(%d))....count : %d\n",
			__func__, wlc, vbus, checking_insert_cnt);
...
static void wireless_set_offline_work(struct work_struct *work)
{
	...
	if (!wlc) {
		/*Need to W/R*/
		if (!usb_present && vbus) {
			pr_debug("[WLC] %s : USB is inserted and No SMB349 IRQ\n", __func__);
			pr_debug("[WLC] %s : USB psy is updated to ONLINE !!!\n", __func__);
		...
		if (usb_present || (checking_usb_insert_cnt > MAX_USB_CHECKING_COUNT)) {
			pr_debug("[WLC] %s : USB Already updated or W/O USB (usb(%d) cnt(%d))\n"
			...
			pr_debug("[WLC] %s : Checking USB ........ count : %d\n"
...
static void wireless_inserted(struct unified_wlc_chip *chip)
{
	...
	if (!wake_lock_active(&chip->wireless_chip_wake_lock)) {
		pr_debug("[WLC] %s : ACQAURE wakelock\n", __func__);
...
static void wireless_removed(struct unified_wlc_chip *chip)
{
	...
	pr_debug("[WLC] %s : Wireless psy is updated to OFFLINE !!!\n", __func__);
...
{
	enum lge_boot_mode_type boot_mode = LGE_BOOT_MODE_NORMAL;
...
static irqreturn_t wireless_interrupt_handler(int irq, void *data)
{
	...
	if (chg_state)
		pr_debug("[WLC] %s : I'm on the WLC PAD\n", __func__);
	else
		pr_debug("[WLC] %s : I'm NOT on the WLC PAD\n", __func__);
...
static void wlc_check_suspended_worker(struct work_struct *work)
{
	...
	if (chip->suspended || i2c_suspended) {
		...
		pr_debug("[WLC] resumed. chip->suspended:%d, i2c_suspended:%d\n",
...
static void wireless_eoc_work(struct work_struct *work)
{
	...
	pr_debug("[WLC] %s : Send EPT signal!!\n", __func__);
	msleep(3500);
	gpio_set_value(chip->wlc_full_chg, 0);
	pr_debug("[WLC] %s : Re-enable RX!!\n", __func__);
...
static int unified_wlc_hw_init(struct unified_wlc_chip *chip)
{
	...
	gpio_tlmm_config(GPIO_CFG(chip->wlc_int_gpio, 0, GPIO_CFG_INPUT,
...
static int unified_wlc_probe(struct platform_device *pdev)
{
	 ...
	 */
	if (is_wireless_charger_plugged_internal(chip)) {
		pr_debug("[WLC] %s : I'm on WLC PAD during booting\n ", __func__);
