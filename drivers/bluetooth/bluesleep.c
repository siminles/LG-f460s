...
static inline int bluesleep_can_sleep(void)
{
	...
	BT_DBG("btwake = %d, hostwake = %d, uport = %p",gpio_get_value(bsi->ext_wake),gpio_get_value(bsi->host_wake),bsi->uport );
...
/*                                                      */
		...
		BT_DBG("btwake = %d, hostwake = %d",gpio_get_value(bsi->ext_wake),gpio_get_value(bsi->host_wake));
#endif /*                      */
/*                                                      */
...
/*                                                      */
		...
		BT_DBG("btwake = %d, hostwake = %d",wake,host_wake);
 ...
 */
/*
static int bluepower_read_proc_btwake(struct file *file, char __user * buffer, 
...

{
	...
	btwake = gpio_get_value(bsi->ext_wake);		// where kernel 3.04 used to

	BT_DBG("buffer = %p, count = %d, btwake = %d", buffer, count, btwake);
	//btwake = test_bit(BT_TXDATA, &flags) ? 1 : 0;

	len = sprintf(tmpbuf, "btwake:%u\n", btwake);
	BT_DBG("tmpbuf = %s, len = %d", tmpbuf, len);

	return simple_read_from_buffer(buffer, count, ppos, tmpbuf, len);
 ...
 */
static int bluepower_write_proc_btwake(struct file *file, const char __user * buffer,
					size_t count, loff_t * ppos)
{
	char btwake;
	BT_INFO("");
	...
	if (copy_from_user(&btwake, buffer, 1)) {
		return -EFAULT;
	}
...
/*                                             */
		...
		gpio_set_value(bsi->ext_wake, 1);
		BT_DBG("gpio_set_value(bsi->ext_wake = %d", gpio_get_value(bsi->ext_wake));
/*                                               */
/* ADD: 0019639: [F200][BT] Support Bluetooth low power mode */
#ifdef CONFIG_LGE_BLUESLEEP
		bluesleep_tx_idle();
 ...
 * @return The number of bytes written.
 */
static int bluesleep_read_proc_proto(struct file *file, char __user * buffer, 
					size_t count, loff_t * ppos)
{
	unsigned int proto;
    char tmpbuf[10];
 ...
 * <code>errno</code> is set appropriately.
 */
static int bluesleep_write_proc_proto(struct file *file, const char __user * buffer,
					size_t count, loff_t * ppos)
{
	char proto;
...
static int __init bluesleep_init(void)
{
	...
	/* Creating write only "btwake" entry */
	ent =
	    bluesleep_proc_create("btwake", S_IRUGO | S_IWUSR | S_IWGRP,
				  sleep_dir, &btwake_fops);
	if (ent == NULL) {
	...
	/* Creating write only "proto" entry */
	ent =
	    bluesleep_proc_create("proto", S_IRUGO | S_IWUSR | S_IWGRP, sleep_dir,
				  &proto_fops);
	if (ent == NULL) {
