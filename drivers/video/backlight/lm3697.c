...
void lm3697_lcd_backlight_set_level(int level)
{
	...
	if (lge_get_boot_mode() == LGE_BOOT_MODE_QEM_130K) {
		pr_info("%s : 130K is connected. set 0 to turn off\n",
				__func__);
		level = 0;
	}
...
#if CONFIG_LGE_FOTA_SILENT_RESET
...
DEVICE_ATTR(lm3697_bl_level, 0644, lcd_backlight_show_level,
...
static int lm3697_bl_set_current_for_retail(struct lm3697_bl *lm3697_bl,
		u8 imax)
{
	u8 reg[] = { LM3697_REG_IMAX_A, LM3697_REG_IMAX_B, };
...
static int lm3697_bl_parse_dt(struct device *dev, struct lm3697_bl_chip *chip)
{
	...
	for_each_child_of_node(node, child) {
		...
		if (bl_pdata[i].blmap_size) {
			array = kzalloc(sizeof(u32) * bl_pdata[i].blmap_size,
					GFP_KERNEL);
			if (!array) {
			...
			of_property_read_u32_array(child, "blmap", array,
					bl_pdata[i].blmap_size);
			bl_pdata[i].blmap =
				kzalloc(sizeof(u16) * bl_pdata[i].blmap_size,
				GFP_KERNEL);
			if (!bl_pdata[i].blmap) {
...
#ifdef CONFIG_MACH_LGE
	lge_blk_set_level = lm3697_lcd_backlight_set_level;
#endif
