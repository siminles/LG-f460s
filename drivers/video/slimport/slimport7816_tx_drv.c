...
static unsigned char __i2c_read_byte(unsigned char dev,
						unsigned char offset);
static void hardware_power_ctl(unchar enable);


#define sp_tx_aux_polling_enable() sp_write_reg_or(TX_P0, \
			TX_DEBUG1, POLLING_EN)
#define sp_tx_aux_polling_disable() sp_write_reg_and(TX_P0, \
			TX_DEBUG1, ~POLLING_EN)
...
#define system_state_change_with_case(status) \
	do { \
		if (sp_tx_system_state >= status) { \
			pr_info("%s %s : change_case: clean_status:%x,\n", \
			LOG_TAG, __func__, \
	...
	unchar power)
{
	if (power == SP_POWER_ON)
		sp_write_reg_and(TX_P2, SP_POWERD_CTRL_REG,
			(~sp_tx_pd_block));
	else
		sp_write_reg_or(TX_P2, SP_POWERD_CTRL_REG,
			(sp_tx_pd_block));
...
void vbus_power_ctrl(unsigned char ON)
{
	...
	if (ON == 0) {
		...
		sp_write_reg_or(TX_P2, TX_PLL_FILTER5,  P5V_PROTECT_PD
			| SHORT_PROTECT_PD);
		/* xjh add */
	...
	if (AUX_OK == sp_tx_aux_dpcdread_bytes
		...
		if ((c1&0x01) == 0x00) {
					downstream_charging_status =
						FAST_CHARGING;
					sp_write_reg_or(TX_P2, TX_ANALOG_CTRL,
						SHORT_DPDM);
					...
					downstream_charging_status =
						NO_FAST_CHARGING;
					sp_write_reg_and(TX_P2, TX_ANALOG_CTRL,
						~SHORT_DPDM);
...
void xtal_clk_sel(void)
{
	pr_info("%s %s : define XTAL_CLK:  %x\n ", LOG_TAG, __func__,
		(uint)XTAL_CLK_DEF);
	sp_write_reg_and_or(TX_P2, TX_ANALOG_DEBUG2, (~0x3c), 0x3c &
		(XTAL_CLK_DEF << 2));
	...
	case GETTED_CABLE_TYPE:
		...
		case 0x03:
			...
			if (((data_buf[0] == 0x00) && (data_buf[1] == 0x22)
				...
				/* for 7732 */
				/* xjh add for charging */
				if ((data_buf[0] == 0x00) && (data_buf[1]
						== 0x22)
				    && (data_buf[2] == 0xb9) && (data_buf[3]
						== 0x73)
				    && (data_buf[4] == 0x69) && (data_buf[5]
						== 0x76)
				    && (data_buf[6] == 0x61)) {
					downstream_charging_status =
						NO_FAST_CHARGING;
					...
					pr_info("%s %s : Downstream is general "
					 "DP2VGA converter.\n",
					 LOG_TAG, __func__);
				}
			}
			break;
		case 0x02:
			if (AUX_OK == sp_tx_aux_dpcdread_bytes(0x00, 0x04,
					0x00, 8, data_buf)) {
				if ((data_buf[0] == 0xb9) && (data_buf[1]
					== 0x22)
				    && (data_buf[2] == 0x00) && (data_buf[3]
						== 0x00)
				    && (data_buf[4] == 0x00) && (data_buf[5]
						== 0x00)
				    && (data_buf[6] == 0x00)) {
					/* xjh add for charging */
					...
					pr_info("%s %s : Downstream is HDMI "
						"dongle.\n", LOG_TAG, __func__);
				} else {
					cur_cable_type = DWN_STRM_IS_DIGITAL;
					pr_info("%s %s : Downstream is general"
...
unchar sp_tx_get_hdmi_connection(void)
{
	...
	if ((c & 0x41) == 0x41) {
		...
		/* removed by span, no use 20130217 */
		return 1;
	} else
		return 0;
...
static unchar get_edid_detail(unchar *data_buf)
{
	...
	pixclock_edid = ((((uint)data_buf[1] << 8)) | ((uint)data_buf[0]
		& 0xFF));
...
void check_edid_data(unchar *pblock_buf)
{
	...
	for (i = 0; i <= ((pblock_buf[0x7e] > 1) ? 1 : pblock_buf[0x7e]);
		 i++) {
...
static void sp_tx_lvttl_bit_mapping(void)
{
	...
	sp_write_reg_and_or(TX_P2, SP_TX_VID_CTRL2_REG, 0x8c, (colorspace &
		0x60) >> 5 | c);
	/* mdelay(500); */
	if (down_sample_en == 1 && c == IN_BPC_10BIT)
		vid_bit = 3;
	sp_write_reg_and_or(TX_P2, BIT_CTRL_SPECIFIC, 0x00, ENABLE_BIT_CTRL |
		vid_bit << 1);
...
ulong sp_tx_pclk_calc(void)
{
	...
	pr_info("%s %s : PCLK = %d.%d\n", LOG_TAG, __func__,
		(((uint)(str_plck))/10), ((uint)str_plck -
		(((uint)str_plck/10)*10)));
	return str_plck;
}
...
static unchar sp_tx_bw_lc_sel(ulong pclk)
{
	...
	pr_info("%s %s : pixel_clk = %d.%d\n", LOG_TAG, __func__,
		(((uint)(pixel_clk))/10), ((uint)pixel_clk -
		(((uint)pixel_clk/10)*10)));
	...
	if (sp_tx_get_link_bw() != c1) {
		...
		pr_info("%s %s : It is different bandwidth between"
			"sink support and cur video!%.2x\n", LOG_TAG,
			__func__, (uint)c1);
		return 1;
	}
...
void slimport_link_training(void)
{
	...
	case LT_START:
		...
		sp_tx_config_ssc(SSC_DEP_4000PPM);  /* xjh mod */
	...
	case LT_FINISH:
		...
		/* by span 20130217. for one lane case. */
		/* if there is link error, adjust pre-emphsis to check error
...
void sp_tx_set_colorspace(void)
{
	...
	#endif
	{
		...
		case DWN_STRM_IS_ANALOG:
		case DWN_STRM_IS_DIGITAL:
		    /* add DP case by span 20130217 */
			sp_read_reg(TX_P2, SP_TX_VID_CTRL2_REG,
				&color_space);
			/* YCBCR422 */
			if ((color_space & 0x03) == 0x01) {
				...
				sp_read_reg(RX_P1, (HDMI_RX_AVI_DATA00_REG
					+ 3), &c);
				...
				if ((c  == 0x04) || (c == 0x05) ||
					...
					(c == 0x3E) || (c == 0x3F) ||
					(c == 0x40)) {
					sp_read_reg(TX_P2, SP_TX_VID_CTRL5_REG,
						&c);
					c |= CSC_STD_SEL;
					sp_write_reg(TX_P2, SP_TX_VID_CTRL5_REG,
						c);
				} else {
					sp_read_reg(TX_P2, SP_TX_VID_CTRL5_REG,
						&c);
...
void slimport_cable_monitor(void)
{
	...
	if (sp_tx_system_state_bak != sp_tx_system_state
		...
		if (sp_tx_cur_states() > STATE_SINK_CONNECTION) {
			cur_cable_type = sp_tx_get_cable_type(
				GETTED_CABLE_TYPE, 0);
			/* cable changed */
...
{
/*	 if((hdmi_system_state < HDMI_CLOCK_DET)||(sp_tx_system_state
		<STATE_CONFIG_HDMI))
		return;
*/
...
static void sp_tx_sink_irq_int_handler(void)
{
	...
	if (IRQ_Vector & CP_IRQ) {
		...
		#endif
		{
			...
			if (c1 & 0x04) {
				...
				sp_tx_clean_hdcp_status();
				#endif
				pr_info("%s %s : IRQ:____________HDCP Sync"
					"lost!\n", LOG_TAG, __func__);
			}
		}
	}
		...
		if (sp_tx_rx_type == DWN_STRM_IS_HDMI_7730) {
			...
			do {
				...
				case 0x02:
					...
					break;
				case 0x04:
					pr_info("%s %s : Here is check: Link "
						"is down!\n", LOG_TAG,
						__func__);
				...
				case 0x08:
					if ((Int_vector[0] & 0x10) != 0x10)
						...
						if (sp_tx_system_state >= STATE_HDCP_AUTH
							...
							sp_tx_set_sys_state(
								STATE_HDCP_AUTH);
							pr_info("%s %s : Re-authentication"
								" due to down stream HDCP "
								"failure!\n",
								LOG_TAG,
								__func__);
						    sp_tx_clean_hdcp_status();
				...
				case 0x10:
					break;
				case 0x20:
					pr_info("%s %s :  Downstream HDCP link "
					...
					system_state_change_with_case(
						STATE_HDCP_AUTH);
					/* msleep(2000);*/
					/* 20130217 by span for */
					/* samsung monitor */
					sp_tx_clean_hdcp_status();
					#endif
					pr_info("%s %s : IRQ:____________"
						"HDCP Sync lost!\n", LOG_TAG,
						__func__);
					break;
				case 0x40:
					pr_info("%s %s : Receive CEC"
						"command from downstream"
						" done!\n", LOG_TAG,
						__func__);
					break;
				case 0x80:
					pr_info("%s %s : CEC command "
	...
	if (IRQ_Vector & TEST_IRQ) {
		...
		if (test_vector & 0x08) { /* phy test pattern */
			...
			while ((c & 0x03) == 0) {
				c = c | 0x01;
				sp_tx_aux_dpcdread_bytes(0x00, 0x02, 0x60, 1,
					&c);
				sp_tx_aux_dpcdread_bytes(0x00, 0x02, 0x60, 1,
					&c);
			}*/
...
/*	unchar temp; */
	...
	slimport_set_hdmi_hpd(0);
#endif
	/* if (AUX_ERR == sp_tx_aux_dpcdread_bytes(0x00, 0x00, 0x00, 1,
		&temp)) */
		system_power_ctrl(0);
}
...
static void hdmi_rx_new_vsi_int(void)
{
	...
	if (!(c&INFO_FRAME_VSC_EN)) {
		...
		sp_read_reg(RX_P1, HDMI_RX_MPEG_DATA03_REG,
			&hdmi_video_format);
		if ((vsi_header == 0x81) && ((hdmi_video_format & 0xe0)
				== 0x40)) {
			pr_info("%s %s : 3D VSI packet is detected. Config VSC"
				"packet\n", LOG_TAG, __func__);
...
static void hdmi_rx_no_vsi_int(void)
{
	...
	sp_read_reg(TX_P0, SP_TX_3D_VSC_CTRL, &c);
	if (c&INFO_FRAME_VSC_EN) {
		pr_info("%s %s : No new VSI is received, disable"
			"VSC packet\n", LOG_TAG, __func__);
		c &= ~INFO_FRAME_VSC_EN;
		sp_write_reg(TX_P0, SP_TX_3D_VSC_CTRL, c);
		sp_tx_mpeg_setup();
...
void system_isr_handler(void)
{
	...
	if (sp_tx_system_state >= STATE_VIDEO_OUTPUT) {
		...
		*/
		...
		if ((HDMI_RX_INT6 & NEW_AUD) || (HDMI_RX_INT3 &
				AUD_MODE_CHANGE))
...
void sp_tx_show_infomation(void)
{
	...
	pr_info("%s %s : HF = %d, HSW = %d, HBP = %d\n", LOG_TAG,
		__func__, h_fp, h_sw, h_bp);
	pr_info("%s %s : VF = %d, VSW = %d, VBP = %d\n", LOG_TAG,
		__func__, v_fp, v_sw, v_bp);
	pr_info("%s %s : Active resolution is %d * %d ", LOG_TAG,
		__func__, h_act, v_act);
...
bool source_aux_read_7730dpcd(long addr, unchar cCount, unchar *pBuf)
{
	...
	c = 0;
	while (1) {
		if (sp_tx_aux_dpcdread_bytes(addr_h, addr_m, addr_l, cCount,
			pBuf) == AUX_OK)
...
bool source_aux_write_7730dpcd(long addr, unchar cCount, unchar *pBuf)
{
	...
	c = 0;
	while (1) {
		if (sp_tx_aux_dpcdwrite_bytes(addr_h, addr_m, addr_l, cCount,
			pBuf)
			== AUX_OK)
			return SOURCE_AUX_OK;
		c++;
...
bool i2c_master_read_reg(unchar Sink_device_sel, unchar offset,
	unchar *Buf)
{
	unchar sbytebuf[2] = {0};
	long a0, a1;
...
bool i2c_master_write_reg(unchar Sink_device_sel, unchar offset,
	unchar value)
{
	unchar sbytebuf[3] = {0};
	long a0;
