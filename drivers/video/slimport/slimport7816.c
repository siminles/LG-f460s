...
static irqreturn_t anx7816_cbl_det_isr(int iqr, void *data);
		...
		size_t count)
{
	...
	if (count != 7 && count != 5) {
		...
		pr_err("%s: ex) 05df   -> op:0(read)  id:5 reg:0xdf\n",
				LOG_TAG);
		pr_err("%s: ex) 15df5f -> op:1(wirte) id:5 reg:0xdf val:"
				"0x5f\n", LOG_TAG);
		return -EINVAL;
	}
		...
		const char *buf, size_t count)
{
	...
	if (count != 7 && count != 5) {
		...
		pr_err("%s: ex) 05df   -> op:0(read)  id:5 reg:0xdf\n",
				LOG_TAG);
		pr_err("%s: ex) 15df5f -> op:1(wirte) id:5 reg:0xdf"
				"val:0x5f\n", LOG_TAG);
		return -EINVAL;
	}
...
static struct device_attribute slimport_device_attrs[] = {
	...
	__ATTR(hdmi_vga, S_IRUGO, slimport_sysfs_rda_hdmi_vga,
			NULL),
...
#ifdef SP_REGISTER_SET_TEST /*slimport test */
	__ATTR(ctrl_reg0, S_IRUGO | S_IWUSR, ctrl_reg0_show,
			ctrl_reg0_store),
	__ATTR(ctrl_reg10, S_IRUGO | S_IWUSR, ctrl_reg10_show,
			ctrl_reg10_store),
	__ATTR(ctrl_reg11, S_IRUGO | S_IWUSR, ctrl_reg11_show,
			ctrl_reg11_store),
	__ATTR(ctrl_reg2, S_IRUGO | S_IWUSR, ctrl_reg2_show,
			ctrl_reg2_store),
	__ATTR(ctrl_reg12, S_IRUGO | S_IWUSR, ctrl_reg12_show,
			ctrl_reg12_store),
	__ATTR(ctrl_reg1, S_IRUGO | S_IWUSR, ctrl_reg1_show,
			ctrl_reg1_store),
	__ATTR(ctrl_reg6, S_IRUGO | S_IWUSR, ctrl_reg6_show,
			ctrl_reg6_store),
	__ATTR(ctrl_reg16, S_IRUGO | S_IWUSR, ctrl_reg16_show,
			ctrl_reg16_store),
	__ATTR(ctrl_reg5, S_IRUGO | S_IWUSR, ctrl_reg5_show,
			ctrl_reg5_store),
	__ATTR(ctrl_reg8, S_IRUGO | S_IWUSR, ctrl_reg8_show,
			ctrl_reg8_store),
		...
		const struct i2c_device_id *id)
{
	...
	/* INIT_DELAYED_WORK(&anx7816->dwc3_ref_clk_work,
	   dwc3_ref_clk_work_func); */
