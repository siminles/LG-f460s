...
static bool mdss_mdp_is_pipe_uhd(struct mdss_mdp_pipe *pipe)
{
	if (pipe->src_fmt->is_yuv &&
		(pipe->img_width > 2560 || pipe->img_height > 2560))
		return true;
	else
		return false;
}
	...
	bool apply_fudge, bool is_single_layer)
{
	...
	if (mixer->ctl->is_video_mode)
		perf->is_uhd = mdss_mdp_is_pipe_uhd(pipe);

	if (is_single_layer && !perf->is_uhd)
		perf->prefill_bytes =
	...
	pr_debug("mixer=%d pnum=%d clk_rate=%u bw_overlap=%llu prefill=%d %s\n",
			mixer->num, pipe->num, perf->mdp_clk_rate, perf->bw_overlap,
			perf->prefill_bytes, perf->is_uhd ? "is_uhd" : "");
	return 0;
}
...
exit:
	pr_debug("final mixer=%d video=%d clk_rate=%u bw=%llu prefill=%d %s\n",
		mixer->num, mixer->ctl->is_video_mode, perf->mdp_clk_rate,
		perf->bw_overlap, perf->prefill_bytes, is_uhd ? "is_uhd" : "");
}
		...
		struct mdss_mdp_perf_params *perf)
{
	...
	pr_debug("bw_overlap=%llu bw_prefill=%llu prefill_bytes=%d %s\n",
...
static inline void mdss_mdp_ctl_perf_update_bus(struct mdss_mdp_ctl *ctl)
{
	...
	if (IS_MDSS_MAJOR_MINOR_SAME(mdata->mdp_rev, MDSS_MDP_HW_REV_103) &&
		...
		if (mdata->perf_tune.min_uhd_bus_vote > 0)
			...
			bus_ib_quota = max(bus_ib_quota, (u64)5000000000ULL);
	}

	mdss_mdp_bus_scale_set_quota(bus_ab_quota, bus_ib_quota);
	pr_debug("ab=%llu ib=%llu %s\n", bus_ab_quota, bus_ib_quota,
		is_uhd ? "is_uhd" : "");
}
		...
		int params_changed)
{
	...
	if (ctl->power_on) {
		...
		if ((params_changed && (new->bw_ctl > old->bw_ctl)) ||
			...
			new->is_uhd) {
			pr_debug("c=%d p=%d new_bw=%llu,old_bw=%llu %s\n",
				ctl->num, params_changed, new->bw_ctl,
				old->bw_ctl, new->is_uhd ? "is_uhd" : "");
			old->bw_ctl = new->bw_ctl;
			old->is_uhd = new->is_uhd;
			update_bus = 1;
		}
