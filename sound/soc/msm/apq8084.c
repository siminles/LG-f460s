...
#ifdef CONFIG_SND_SOC_CS35L32
#define GPIO_QUAT_MI2S_MCLK   91
#define GPIO_QUAT_MI2S_SCK    92
#define GPIO_QUAT_MI2S_WS     93
#define GPIO_QUAT_MI2S_DATA0  94
#ifdef CONFIG_SND_SOC_TPA2015D
bool tpa_enabled;
#endif
...
#ifdef CONFIG_SND_SOC_CS35L32
static int apq8084_quat_mi2s_free_gpios(void)
{
	int i;
	pr_info("call %s\n", __func__);
...
static int apq8084_mclk_mi2s_free_gpios(void)
{
	int i;
	pr_info("call %s\n", __func__);
...
static void apq8084_mi2s_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (atomic_dec_return(&quat_mi2s_clk.mi2s_rsc_ref) == 0) {
		...
		apq8084_quat_mi2s_free_gpios();
	}
}

static int apq8084_configure_quat_mi2s_gpio(void)
{
	int rtn;
	int i;
...
static int apq8084_configure_mclk_mi2s_gpio(void)
{
	int rtn;
	int i;
	pr_info("call %s\n", __func__);
...
static int apq8084_mi2s_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&quat_mi2s_clk.mi2s_rsc_ref) == 1) {
		pr_info("%s: aquire mi2s resources\n", __func__);
		apq8084_configure_quat_mi2s_gpio();
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (clk == 1) { /*mclk on*/
		if (atomic_inc_return(&quat_mi2s_clk.mi2s_rsc_ref) == 1) {
			...
			apq8084_configure_mclk_mi2s_gpio();
			ret = afe_set_lpass_clock(AFE_PORT_ID_QUATERNARY_MI2S_TX,
				&lpass_mi2s_enable);
			if (ret < 0) {
				pr_err("%s: afe_set_lpass_clock failed\n", __func__);
...
#endif
#ifdef CONFIG_SND_SOC_CS35L32
	SOC_ENUM_EXT("Speaker Protection", msm_snd_enum[11],
...
#ifdef CONFIG_SND_SOC_CS35L32
static struct snd_soc_dai_link apq8084_cirrus_dai_link[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
#ifdef CONFIG_SND_SOC_CS35L32
	if (lge_get_board_revno() >= HW_REV_CIRRUS) {
		dev_dbg(dev, "%s(): cirrus amp support present\n",
				__func__);

		memcpy(dailink + card->num_links, apq8084_cirrus_dai_link,
				sizeof(apq8084_cirrus_dai_link));
		card->dai_link = dailink;
		card->num_links += ARRAY_SIZE(apq8084_cirrus_dai_link);
	}
#endif
	return card;
}
...
static int apq8084_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_available(tpa_amp_node))
		tpa_enabled = true;
	else
		tpa_enabled = false;
#endif
