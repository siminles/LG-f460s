...
static int msm_pcie_sys_handler(struct notifier_block *nb,
		unsigned long action, void *data)
{
	struct msm_pcie_dev_t *dev = container_of(nb, struct msm_pcie_dev_t,
			nb);

	if (dev->rc_idx != 1)
		return NOTIFY_OK;

	PCIE_INFO(dev, "[BSP-PCIE] %s: RC%d action %lu\n", __func__,
			dev->rc_idx, action);

	PCIE_INFO(dev, "[BSP-PCIE] %s: disable linkdown irq.\n", __func__);
	disable_irq(dev->irq[MSM_PCIE_INT_LINK_DOWN].num);
	PCIE_INFO(dev, "[BSP-PCIE] %s: disable wake irq.\n", __func__);
...
#ifdef CONFIG_LGE_MSM_PCIE
	dev->nb.notifier_call = msm_pcie_sys_handler;
	rc = register_reboot_notifier(&dev->nb);
	if (rc)
		PCIE_ERR(dev, "PCIe: RC%d: register for reboot failed\n",
				dev->rc_idx);
#endif
